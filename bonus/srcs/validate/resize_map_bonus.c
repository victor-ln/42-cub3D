...
static int	is_garbage(char **map, t_validation v)
{
	if (map[v.line][v.counter] == '0' || \
		(!v.counter || ft_strchr("0NEWSDe", map[v.line][v.counter - 1])))
		return (0);
	if (v.line)
		if (v.counter <= v.column_limit[TOP_LINE])
			if (ft_strchr("0NEWSDe", map[v.line - 1][v.counter]) || \
				ft_strchr("0NEWSDe", map[v.line - 1][v.counter - 1]))
				return (0);
	if (v.line < v.total_lines)
...
static int	just_walls_and_spaces(char *map_line)
{
	...
	i = 0;
	while (map_line[i])
		if (ft_strchr("0NEWSDe", map_line[i++]))
			return (0);
	return (1);
}
